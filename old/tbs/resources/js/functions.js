...
var map;
var chars = new Array();
var guyList = new Array();
var cpos = new Array();
var canMove = true;
var animTime = 0;
var drawDirection = 2+1; //default direction 0=N 1=E 2=S 3=W +1 for stupidity
...
function getChars() {
	//name,map,x,y
	var charsOld = new Array(); //previous chars for comparison
	if (chars) { charsOld = chars.slice(0); }
	var gc = $.get("resources/php/getCharsOnMap.php?m="+cpos[0]).success(function() {
		...
		for (var i = 0; i < chars.length; i++) {
			...
			var name = chars[i][0];
			for (var j = 0; j < charsOld.length; j++) {
				//find the person's name in old list
				if (charsOld[j].indexOf(name.toString()) == 0) {
					//compare positions 0=N 1=E 2=S 3=W
					// +1 because javascript is stupid
					if (chars[i][2] > charsOld[j][2]) {
						drawDirection = 1+1; //E
					} else if (chars[i][2] < charsOld[j][2]) {
						drawDirection = 3+1; //W
					} else if (chars[i][3] > charsOld[j][3]) {
						drawDirection = 2+1; //S
					} else if (chars[i][3] < charsOld[j][3]) {
						drawDirection = 0+1; //N
					} else {
						if (charsOld[j][5]) { drawDirection = charsOld[j][5]; }
					}
					chars[i].push(drawDirection);
					debug = "old: "+charsOld.toString()+"\nnew: "+chars.toString();
					j = charsOld.length; //cancel loop if found
				}
			}
...
function drawChars() {
	if (chars) {
		...
		for (var i = 0; i < chars.length; i++) {
			...
			var CHAR_IMG = new Image();
			CHAR_IMG.src = "resources/images/"+chars[i][4]+".png";
			if (chars[i][0]!=user) {
...
function move(vx,vy) {
	...
	dest = dest.join("|");
	var result = $.ajax("resources/php/move.php?s="+sid+"&k="+kid+"&d="+dest).success(function() {
